...
function GeralReport({ kpis, leads, vendedores, isAdmin }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      <Card>
        <div style={{ fontSize: 16, fontWeight: 700, color: T.text, marginBottom: 20 }}>Totais acumulados</div>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 0 }}>
          {rows.map((r, i) => (
            <div key={r.key} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 0', borderBottom: i < rows.length - 1 ? '1px solid ' + T.border : 'none' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
...
export default function TabRelatorios({ kpis, leads, viewLabel, isAdmin, vendedores, teamKpis, teamLeads }) {
  ...
  const ABAS = isAdmin
    ? [{ key: 'mensal', label: 'Histórico Mensal' }, { key: 'geral', label: 'Geral' }]
    : [{ key: 'mensal', label: 'Histórico Mensal' }];
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
      ...
      {aba === 'geral' && isAdmin && (
        <Section title="Geral" subtitle="Soma acumulada de todos os períodos">
          <GeralReport
            kpis={teamKpis || kpis}
            leads={teamLeads || leads}
